...
@tf.function
def tf_ensure_grayscale(image: tf.Tensor) -> tf.Tensor:
    """Convert image to grayscale if in RGB format.
    ...
    Notes:
        This function is autographed and relies on the dynamic shape of the tensor.
...
@tf.function
def tf_ensure_rgb(image: tf.Tensor) -> tf.Tensor:
    """Convert image to RGB if in grayscale format.
    ...
    def transform_dataset(self, ds_input: tf.data.Dataset) -> tf.data.Dataset:
        ...
        def normalize(example):
            ...
            if self.imagenet_mode == "tf":
                img = scale_to_imagenet_tf_mode(img)

            if self.imagenet_mode == "caffe":
                img = scale_to_imagenet_caffe_mode(img)

            if self.imagenet_mode == "torch":
                img = scale_to_imagenet_torch_mode(img)
